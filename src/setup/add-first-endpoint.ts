...
export const runAddFirstEndpoint = async (
  ...
  const pick = await vscode.window.showInformationMessage(
    `AI Normalizer: added endpoint "${endpoint.displayName ?? endpoint.id}". Set your upstream API key next.`,
    "Set API Key",
    "Open Settings",
    "Later"
  );
  if (pick === "Set API Key" || options?.promptForApiKey) {
    await vscode.commands.executeCommand("aiNormalizer.setEndpointApiKey");
