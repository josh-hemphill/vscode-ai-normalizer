...
export const pickEndpoint = async (
  ...
  if (endpoints.length === 0) {
    void vscode.window.showWarningMessage(
      "Configure aiNormalizer.endpoints before setting an API key."
    );
    return undefined;
  }
...
export const promptAndStoreEndpointApiKey = async (
  ...
  if (!ep.apiKeySecretId?.trim()) {
    ...
    await vscode.workspace
      .getConfiguration("aiNormalizer")
      .update("endpoints", endpoints, vscode.ConfigurationTarget.Global);
  }

  void vscode.window.showInformationMessage(
    `AI Normalizer: API key saved for "${ep.displayName ?? ep.id}". Run Refresh Model Catalog.`
  );
  return true;
};
...
export const clearEndpointApiKey = async (
  ...
  void vscode.window.showInformationMessage(
    `AI Normalizer: API key cleared for "${ep.displayName ?? ep.id}".`
  );
  return true;
};
