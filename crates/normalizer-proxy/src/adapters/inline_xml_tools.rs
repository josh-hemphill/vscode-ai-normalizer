...
impl InlineXmlToolsAdapter {
    pub async fn chat_completions(
        ...
        let normalized = transform_non_stream_body(&text, &request.model)?;
        Ok(Response::builder()
...
fn transform_non_stream_body(raw: &str, model: &str) -> Result<String, AdapterError> {
        ...
        if let Some(choices) = parsed.get_mut("choices").and_then(|c| c.as_array_mut()) {
            for choice in choices {
                if let Some(msg) = choice.get_mut("message") {
                    normalize_message_value(msg, &ToolFormatProfile::default());
                }
            }
        }
        return Ok(serde_json::to_string(&parsed)?);
    }
    let (content, extracted_calls) = extract_from_text(raw, &ToolFormatProfile::default());
...
fn take_sse_data_event(buffer: &mut String) -> Option<String> {
...
{"location":"Tokyo"}
    ...
    fn transform_non_stream_parses_xml_in_body() {
        ...
        let out = transform_non_stream_body(raw, "test-model").expect("transform");
